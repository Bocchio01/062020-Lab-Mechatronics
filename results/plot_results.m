...
tiles = tiledlayout(2, 3 * not(is_single_controller) + 2 * (is_single_controller), ...
...
ylim([0.9*min(ref) 1.05*max(ref)]*1000)
...
% Controls data
controls_tile = nexttile(tiles, 4 * not(is_single_controller) + 4 * (is_single_controller));
hold on
grid on
...
for simout_idx = 1:size(simouts, 1)
    ...
    % Position data
    nexttile(tiles, 1);
    plot(time, z_hat * 1000, 'Color', [colorscheme(mod(simout_idx, size(colorscheme, 1)), :) alpha], 'DisplayName', legend_name)
    ...
    % Controls data
    nexttile(tiles, 4 * not(is_single_controller) + 4 * (is_single_controller));
    % % For controller comparison use these lines
